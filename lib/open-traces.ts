...
const TRACE_RECORDS: TraceRecord[] = [
  ...
  // cuDNN 9.19 · sm90a · the same xmma implicit-GEMM fprop family at a 64-wide
  // k-tile, reached by a 3x3 convolution over 256 channels at 28x28
  {
    vendor: "NVIDIA",
    software: "cuDNN",
    version: "9.19.0",
    arch: "sm90a",
    kernel: "sm90_xmma_fprop_implicit_gemm 128x128x64 segment_k_off",
    params: {
      api: "cudnnConvolutionForward",
      n: 8,
      c: 256,
    ...
    call: [
      ...
      "  handle,",
      "  alpha=1.0f,",
      "  xDesc={NHWC, HALF, n=8, c=256, h=28, w=28}, x,",
      "  wDesc={NHWC, HALF, k=256, c=256, r=3, s=3}, w,",
      "  convDesc={pad=1x1, stride=1x1, dilation=1x1,",
      "            CROSS_CORRELATION, computeType=FLOAT},",
      "  algo=CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM,",
      "  workspace, workspaceSize,",
      "  beta=0.0f,",
      "  yDesc={NHWC, HALF, n=8, c=256, h=28, w=28}, y",
      ")",
    ].join("\n"),
    meta: { GPU: "NVIDIA H100 80GB HBM3" },
    gwatchVersion: "0.0.33",
    trace: "/traces/cudnn-xmma-fprop-128x128x64-n8c256hw28-fp16-nhwc.json",
  },
  ...
  // FlashAttention-3 · sm90a · the mid-sequence point of the seqlen sweep:
  // 256 tiles, 11 in-loop KV iterations per tile
  {
    vendor: "NVIDIA",
    software: "FlashAttention-3",
    version: "3.0.0b1",
    ...
    meta: { GPU: "NVIDIA H100 80GB HBM3" },
    gwatchVersion: "0.0.33",
    trace: "/traces/flash-attention-3-fwd-sm90-bf16-hdim128-b2s2048h8.json",
  },
  // FlashAttention-3 · sm90a · the extreme-context shape: 128 tiles, one per
  // CTA, and a 46-iteration KV loop stretches one staging pass to ~90 µs
  {
    vendor: "NVIDIA",
    software: "FlashAttention-3",
    version: "3.0.0b1",
    ...
    meta: { GPU: "NVIDIA H100 80GB HBM3" },
    gwatchVersion: "0.0.33",
    trace: "/traces/flash-attention-3-fwd-sm90-bf16-hdim128-b1s8192h2.json",
  },
  // FlashAttention-3 · sm90a · the largest batch: 2048 tiles, ~16 per CTA,
  // the steady-state pipeline sustained for ~220 µs
  {
    vendor: "NVIDIA",
    software: "FlashAttention-3",
    version: "3.0.0b1",
    ...
    meta: { GPU: "NVIDIA H100 80GB HBM3" },
    gwatchVersion: "0.0.33",
    trace: "/traces/flash-attention-3-fwd-sm90-bf16-hdim128-b16s1024h16.json",
  },
  // FlashAttention-3 · sm90a · the head sweep: 32 heads at the base shape
  {
    vendor: "NVIDIA",
    software: "FlashAttention-3",
    version: "3.0.0b1",
    ...
    call: [
      "flash_attn_func(",
      "  # B=2, S=1024, H=32, D=128, precision=bf16",
      "  q, k, v,",
      "  softmax_scale=None,    # 1/sqrt(128)",
      "  causal=False,",
      "  window_size=(-1, -1),",
    ...
    meta: { GPU: "NVIDIA H100 80GB HBM3" },
    gwatchVersion: "0.0.33",
    trace: "/traces/flash-attention-3-fwd-sm90-bf16-hdim128-b2s1024h32.json",
  },
  // FlashAttention-3 · sm90a · the batch extreme at short sequence: 1024
  // two-iteration tiles, throughput-bound
  {
    vendor: "NVIDIA",
    software: "FlashAttention-3",
    version: "3.0.0b1",
    ...
    call: [
      "flash_attn_func(",
      "  # B=32, S=512, H=8, D=128, precision=bf16",
      "  q, k, v,",
      "  softmax_scale=None,    # 1/sqrt(128)",
      "  causal=False,",
      "  window_size=(-1, -1),",
    ...
    meta: { GPU: "NVIDIA H100 80GB HBM3" },
    gwatchVersion: "0.0.33",
    trace: "/traces/flash-attention-3-fwd-sm90-bf16-hdim128-b32s512h8.json",
  },
  // FlashAttention-3 · sm90a · batch x sequence combined: 1024 tiles of 11
  // KV iterations each, ~8 per CTA
  {
    vendor: "NVIDIA",
    software: "FlashAttention-3",
    version: "3.0.0b1",
    ...
    call: [
      "flash_attn_func(",
      "  # B=4, S=2048, H=16, D=128, precision=bf16",
      "  q, k, v,",
      "  softmax_scale=None,    # 1/sqrt(128)",
      "  causal=False,",
      "  window_size=(-1, -1),",
    ...
    meta: { GPU: "NVIDIA H100 80GB HBM3" },
    gwatchVersion: "0.0.33",
    trace: "/traces/flash-attention-3-fwd-sm90-bf16-hdim128-b4s2048h16.json",
  },
  // FlashAttention-3 · sm90a · the fp16 twin of the base shape: the kernel
  // is instruction-for-instruction the bf16 one with half_t types
  {
    vendor: "NVIDIA",
    software: "FlashAttention-3",
    version: "3.0.0b1",
    arch: "sm90a",
    kernel: "flash_fwd_sm90 fp16 hdim128 tile 128x176",
    params: {
      api: "flash_attn_func",
      batch: 2,
      seqlen: 1024,
    ...
    call: [
      "flash_attn_func(",
      "  # B=2, S=1024, H=8, D=128, precision=fp16",
      "  q, k, v,",
      "  softmax_scale=None,    # 1/sqrt(128)",
      "  causal=False,",
      "  window_size=(-1, -1),",
    ...
    meta: { GPU: "NVIDIA H100 80GB HBM3" },
    gwatchVersion: "0.0.33",
    trace: "/traces/flash-attention-3-fwd-sm90-fp16-hdim128-b2s1024h8.json",
  },
  // FlashAttention-3 · sm90a · head_dim 64: a different kernel entirely
  // (tile 192x128, three math warpgroups, warp 15 issuing the O store,
  // warp 0 the sole TMA producer)
  {
    vendor: "NVIDIA",
    software: "FlashAttention-3",
    version: "3.0.0b1",
    arch: "sm90a",
    kernel: "flash_fwd_sm90 bf16 hdim64 tile 192x128",
    params: {
      api: "flash_attn_func",
      batch: 2,
      seqlen: 1024,
    ...
    meta: { GPU: "NVIDIA H100 80GB HBM3" },
    gwatchVersion: "0.0.33",
    trace: "/traces/flash-attention-3-fwd-sm90-bf16-hdim64-b2s1024h8.json",
  },
];
